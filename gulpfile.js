...
function html(done) {
  src(path.src.html).pipe(rigger()).pipe(dest(path.dev.html)).pipe(browsersync.stream());
  done();
}
...
function img(done) {
  src(path.src.img).pipe(dest(path.dev.img));
  done();
}

function fonts(done) {
  src(path.src.fonts).pipe(dest(path.dev.fonts));
  done();
}
